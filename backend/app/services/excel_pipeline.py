"""Shared Excel-upload pipeline.

Extracted from `routers/excel.py::upload_excel` so both the synchronous
HTTP handler and the Celery worker can reuse the exact same logic.

The pipeline takes raw XLSX bytes + a user identity + a user-scoped-or-
admin SQLAlchemy session and returns a fully-formed
`ExcelUploadResponse` dict.  It does not know whether it's running
inside a web request or a worker.

Contract:
  * `db` must accept `execute()` against `usage_counters`, `users`, and
    `upload_history`.  The production call paths supply a user-scoped
    (RLS) session from FastAPI, or an admin session from the worker.
    Billing SQL scopes every read/write to `:uid` explicitly so either
    works.
  * `audit` logging is best-effort; pass `request=None` in worker paths
...
from app.services.excel_import import parse_direct_excel_bytes
...
def _maybe_paywall_to_schema(pw: billing.Paywall | None) -> Paywall | None:
    if pw is None:
        return None
    return Paywall(
        trigger=pw.trigger,
...
def _campaigns_to_schema(raw: list[dict[str, Any]]) -> list[CampaignAnalytics]:
    return [
        CampaignAnalytics(
            name=c["name"],
            groups=c["groups"],
...
async def run_upload_pipeline(
    *,
    ...
    """Run the full upload pipeline and return the response payload.

    Mirrors the product flow documented in routers/excel.upload_excel —
    see that module for the narrative.
    """
    max_bytes = max_bytes or settings.max_upload_size_bytes
    audit_target = audit_user if audit_user is not None else user_id

    usage = await billing.get_usage(db, user_id, plan_id, lifetime_ai_ads)
    ...
    if usage.uploads_exhausted:
        ...
        await audit.log(
            ...
            user=audit_target,
            success=False,
            meta={"filename": filename, "reason": "upload_limit"},
        )
        return _paywall_only_response(empty_summary, [], plan_id, usage, pw)

    # --- Parse --------------------------------------------------------
    parsed = await parse_direct_excel_bytes(file_bytes, max_bytes=max_bytes)
    ...
    if not ads_raw:
        await billing.consume_usage(db, user_id, uploads=1, plan_id=plan_id)
        await audit.log(
            action=audit.Action.EXCEL_UPLOAD,
            request=audit_request,
            user=audit_target,
            success=False,
            meta={"filename": filename, "ads": 0, "errors": len(errors_raw)},
        )
        usage_after = await billing.get_usage(
            db, user_id, plan_id, lifetime_ai_ads
        )
        return _paywall_only_response(
            empty_summary, errors_raw, plan_id, usage_after, None
        )

    # --- Gate 2: per-upload ads cap -----------------------------------
    total_parsed = len(ads_raw)
    kept, _trimmed = billing.cap_ads_per_upload(total_parsed, plan_id)
    ...
    improvement_tasks = {
        idx: asyncio.create_task(ai_ads.improve_ad(ads_kept[idx]))
    ...
    usage_after = await billing.get_usage(
        db, user_id, plan_id, lifetime_ai_ads + summary.improved_count
    )
